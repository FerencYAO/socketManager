...
- (BOOL)connectToHost:(NSString *)host onPort:(UInt16)port error:(NSError **)errPtr
{
	...
	int error = [self convertForSendHost:host port:port intoAddress4:&address4];
	if(error)
	{
		if(errPtr)
		{
...
- (BOOL)sendData:(NSData *)data toHost:(NSString *)host port:(UInt16)port withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	NSData *address4 = nil;
	[self convertForSendHost:host port:port intoAddress4:&address4];
...
{
	NSParameterAssert(sock == theSocket4);
